...
[data-testid="stSidebar"] { background: #ffffff; border-right: 1px solid #e2e8f0; box-shadow: 2px 0 8px rgba(0,0,0,0.05); }
.app-header { background: linear-gradient(135deg, #4f46e5 0%, #6366f1 60%, #818cf8 100%); border-radius: 16px; padding: 2rem 2.5rem 1.8rem; margin-bottom: 1.5rem; box-shadow: 0 4px 20px rgba(79,70,229,0.25); }
...
.app-badge { display:inline-block;background:rgba(255,255,255,.25);color:#fff;font-size:.68rem;font-weight:600;padding:3px 12px;border-radius:20px;letter-spacing:.08em;text-transform:uppercase;margin-bottom:.6rem;border:1px solid rgba(255,255,255,.35); }
.vcard { border-radius:12px;padding:1.2rem 1.6rem;margin:.8rem 0;border-left:5px solid;background:#fff;box-shadow:0 2px 8px rgba(0,0,0,.06); }
...
.stat-box { background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:.75rem 1rem;box-shadow:0 1px 4px rgba(0,0,0,.05); }
.stat-lbl { font-size:.65rem;color:#94a3b8;text-transform:uppercase;letter-spacing:.1em;font-family:'JetBrains Mono',monospace;margin-bottom:.2rem; }
...
.risk-badge { display:inline-block;padding:3px 14px;border-radius:20px;font-size:.72rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase; }
.mode-banner { border-radius:10px;padding:.8rem 1.2rem;margin:.8rem 0;font-size:.82rem;font-weight:600; }
...
.stButton>button { background:#4f46e5;color:#fff;border:none;border-radius:10px;font-family:'Plus Jakarta Sans',sans-serif;font-weight:600;padding:.6rem 1.8rem;width:100%;transition:all .2s;box-shadow:0 2px 8px rgba(79,70,229,.3); }
...
.rbox { background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:1.2rem;font-family:'JetBrains Mono',monospace;font-size:.76rem;color:#475569;white-space:pre-wrap;line-height:1.75;max-height:420px;overflow-y:auto; }
.section-lbl { font-size:.7rem;color:#4f46e5;text-transform:uppercase;letter-spacing:.15em;font-family:'JetBrains Mono',monospace;font-weight:600;margin-bottom:.4rem; }
@media (max-width: 768px) {
    .stat-row { grid-template-columns: repeat(2, 1fr) !important; }
    .app-header { padding: 1.2rem !important; }
    .app-header h1 { font-size: 1.2rem !important; }
    .feat-name { width: 130px !important; }
}
...
def feat_bars_html(scores):
    ...
    for k, v in scores.items():
        ...
        rows.append(f'<div class="feat-row"><div class="feat-name">{label}</div>'
                    f'<div class="feat-bar-wrap"><div class="feat-bar" style="width:{v*100:.1f}%;background:{color}"></div></div>'
                    f'<div class="feat-val">{v:.2f}</div></div>')
    return "".join(rows)

def stat_box(label, value, color="#1e293b"):
...
with st.sidebar:
    ...
    <div style='text-align:center;padding:1rem 0 1.5rem'>
        ...
        <div style='color:#6366f1;font-size:.72rem;letter-spacing:.15em;text-transform:uppercase;font-weight:600;margin-top:.3rem'>
            Forensik Citra Digital
        </div>
    </div>""", unsafe_allow_html=True)
    st.markdown("**Mode Analisis**")
    force_mode = st.selectbox("Mode", ["🤖 Otomatis (deteksi jenis gambar)", "📄 Mode Dokumen", "📷 Mode Foto"],
                               label_visibility="collapsed")
    st.divider()
    quality = st.slider("Kualitas Rekompresi JPEG", 50, 95, 90, 5)
    amplify = st.slider("Amplifikasi Visualisasi", 5, 30, 15, 1)
    multi_q = st.multiselect("Kualitas Multi-ELA:", [70, 75, 80, 85, 90, 95], default=[70, 80, 90, 95])
...
uploaded = st.file_uploader("Upload", type=["jpg","jpeg","png","bmp","webp"], label_visibility="collapsed")

if uploaded is None:
    st.markdown("""
    <div style='text-align:center;padding:3.5rem 0;color:#94a3b8'>
      <div style='font-size:3rem'>📂</div>
      <div style='color:#475569;font-size:1rem;margin:.6rem 0'>Upload gambar untuk memulai analisis</div>
      <div style='font-size:.8rem'>Format: JPG · PNG · BMP · WEBP</div>
      <div style='margin-top:1.5rem;font-size:.78rem;color:#64748b;background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:.8rem 1.5rem;display:inline-block;font-family:monospace'>
...
if is_document_mode:
    st.markdown(f'<div class="mode-banner mode-doc">📄 <b>Mode Dokumen Aktif</b> &nbsp;·&nbsp; Terdeteksi: {doc_check.image_type} ({doc_check.confidence:.1%}) &nbsp;·&nbsp; {doc_check.reason}</div>', unsafe_allow_html=True)
else:
    st.markdown(f'<div class="mode-banner mode-photo">📷 <b>Mode Foto Aktif</b> &nbsp;·&nbsp; Terdeteksi: {doc_check.image_type} ({doc_check.confidence:.1%}) &nbsp;·&nbsp; {doc_check.reason}</div>', unsafe_allow_html=True)

if is_document_mode:
    prog.progress(20, "Menganalisis dokumen...")
    ...
    st.markdown(f'<div class="vcard {doc_css}"><h2>{doc_label}</h2><p>{doc_result.summary} &nbsp;<span class="risk-badge {risk_css}">Risk: {doc_result.risk_level}</span></p></div>', unsafe_allow_html=True)

    col_l, col_r = st.columns(2)
    with col_l:
        st.markdown('<div class="section-lbl">Metrik ELA Dokumen</div>', unsafe_allow_html=True)
        susp_color = "#ef4444" if doc_result.ela_suspicious_ratio > 0.05 else "#16a34a"
        st.markdown(f'<div class="stat-row">{stat_box("Mean Error", f"{doc_result.ela_mean_error:.3f}")}{stat_box("Suspicious", f"{doc_result.ela_suspicious_ratio:.2%}", susp_color)}{stat_box("Bg Consist.", f"{doc_result.background_consistency:.3f}")}{stat_box("Edge Anomaly", f"{doc_result.edge_anomaly_score:.3f}")}</div>', unsafe_allow_html=True)
    with col_r:
        st.markdown('<div class="section-lbl">Catatan Analisis</div>', unsafe_allow_html=True)
        for note in doc_result.notes:
            st.markdown(f"- {note}")

    st.markdown('<div class="section-lbl" style="margin-top:1.5rem">Detail Analisis Dokumen</div>', unsafe_allow_html=True)
    ...
    with tab_metrics:
        ...
        **Panduan membaca metrik:**
        ...
        - **Background Inkonsistensi** — tinggi = area putih tidak seragam
        - **Anomali Tepi Teks** — tinggi = teks/angka kemungkinan diedit
        - **Variansi Blok** — tinggi = ada konten asing disisipkan
        - **Lompatan Warna** — tinggi = perubahan warna tidak wajar
        """)
    with tab_report:
    ...
    st.markdown(f'<div class="vcard {ov_css}"><h2>{ov_label}</h2><p>{full.summary} &nbsp;<span class="risk-badge {risk_css}">Risk: {full.risk_level}</span></p></div>', unsafe_allow_html=True)
    ...
    with col_ela:
        ...
        st.markdown(f'<div class="vcard {e_css}" style="padding:1rem 1.4rem"><h2 style="font-size:1.1rem">{e_lbl}</h2><p>Confidence: <b>{ela_result.confidence:.1%}</b></p></div>', unsafe_allow_html=True)
        ec1 = "#ef4444" if ela_result.verdict == "MANIPULATED" else "#16a34a"
        st.markdown(f'<div class="stat-row">{stat_box("Mean Error", f"{ela_result.mean_error:.3f}")}{stat_box("Std Error", f"{ela_result.std_error:.3f}")}{stat_box("Max Error", f"{ela_result.max_error:.1f}")}{stat_box("Suspicious", f"{ela_result.suspicious_ratio:.2%}", ec1)}</div>', unsafe_allow_html=True)
    ...
    with col_ai:
        ...
        st.markdown(f'<div class="vcard {a_css}" style="padding:1rem 1.4rem"><h2 style="font-size:1.1rem">{a_lbl}</h2><p>Confidence: <b>{ai_result.confidence:.1%}</b> &nbsp;·&nbsp; AI Probability: <b>{ai_result.ai_probability:.1%}</b></p></div>', unsafe_allow_html=True)
